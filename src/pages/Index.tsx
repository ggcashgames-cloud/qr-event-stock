...
const Index = () => {
  ...
  const loadProducts = async () => {
    try {
      ...
      const formattedProducts: Product[] = data.map(product => ({
        ...
        minStock: 0,
        // Removed min_stock functionality
  ...
  const loadEventProductCounts = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('event_products').select('event_id, quantity_sent');
      if (error) {
  ...
  const handleSendToEvent = async (productId: string, eventId: string, quantity: number) => {
    try {
      ...
      const {
        error: eventProductError
      } = await supabase.from('event_products').upsert({
        event_id: eventId,
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              ...
              {profile && <p className="text-sm text-muted-foreground">
        ...
        <Tabs defaultValue="estoque" className="w-full">
          ...
          <TabsContent value="estoque" className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex gap-2">
                ...
                {(isAdmin || isFinancialAdmin) && <Button onClick={openNewProductForm} className="flex-1 sm:flex-none">
            ...
            {filteredProducts.length === 0 ? <div className="text-center py-12">
                ...
                {(isAdmin || isFinancialAdmin) && <Button onClick={openNewProductForm}>
                    <Plus className="h-4 w-4 mr-2" />
          ...
          <TabsContent value="enviar" className="space-y-6">
            <SendMaterialScanner products={products} events={events} onSendToEvent={handleSendToEvent} />
          </TabsContent>
          ...
          {isAdmin && <TabsContent value="aprovacoes" className="space-y-6">
              <UserApproval />
            </TabsContent>}
        ...
        {/* Forms and Modals */}
        <ProductForm isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} onSave={handleSaveProduct} product={editingProduct} initialQRData={scannedQRData} />

        <EventForm isOpen={isEventFormOpen} onClose={() => setIsEventFormOpen(false)} onSave={handleSaveEvent} event={editingEvent} />
